  ...
  const doAssign = async (courtId) => {
    try {
      ...
      Toast.show({
        type: "error",
        text1: "Lỗi",
        text2: e?.data?.message || e?.error || "Không thể gán sân",
      });
      onAssigned?.({
  ...
  const clearAssign = async () => {
    try {
      ...
      Toast.show({
        type: "error",
        text1: "Lỗi",
        text2: e?.data?.message || e?.error || "Không thể bỏ gán sân",
      });
      onAssigned?.({
...
/* ===================== MAIN ===================== */
export default function RefereeConsoleScreen() {
  //   console.log(matchId)
  const scrollRef = useRef(null);
  useScrollToTop(scrollRef); // 👈 đặt trong body component
  const navigation = useNavigation();
  const router = useRouter();

  // Luôn khoá ngang khi vào, về dọc khi rời màn
  // Khóa ngang 1 lần khi màn mount. Không unlock ở cleanup để tránh “bật về list”.
  ...
  //      headerShown: true,
  //      headerBackVisible: false,            // ẩn mũi tên mặc định
  //      headerLeft: () => (
  //        <Pressable onPress={handleClose} hitSlop={10} style={{ paddingHorizontal: 8 }}>
  //          <MaterialIcons name="close" size={22} color="#111827" />
  //        </Pressable>
  //      ),
  //      // headerTitle: "Referee", // tuỳ bạn muốn hiển thị gì
  //    });
  ...
  const onStart = async () => {
    ...
    try {
      ...
      Toast.show({
        type: "error",
        text1: "Lỗi",
        text2: e?.data?.message || e?.error || "Không thể start",
      });
    }
  };
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      Toast.show({
        type: "error",
        text1: e?.data?.message || e?.error || "Không thể finish",
      });
    }
  };
  ...
  const onPickWinner = async (w) => {
    ...
    try {
      ...
      Toast.show({ type: "success", text1: `Đã đặt winner: ${w || "—"}` });
    } catch (e) {
      Toast.show({
        type: "error",
        text1: "Lỗi",
        text2: e?.data?.message || e?.error || "Không thể đặt winner",
      });
    }
  };
  ...
  const inc = async (side) => {
    ...
    try {
      ...
      Toast.show({
        type: "error",
        text1: "Lỗi",
        text2: e?.data?.message || e?.error || "Không thể cộng điểm",
      });
    }
  };
  ...
  const dec = async (side) => {
    ...
    try {
      ...
      Toast.show({
        type: "error",
        text1: "Lỗi",
        text2: e?.data?.message || e?.error || "Không thể trừ điểm",
      });
    }
  };
  ...
  const startNextGame = async () => {
    ...
    try {
      ...
      Toast.show({
        type: "error",
        text1: "Lỗi",
        text2: e?.data?.message || e?.error || "Không thể tạo ván mới",
      });
    }
  };
  ...
  const confirmEarlyEnd = async () => {
    ...
    try {
      if (useCurrentScore) {
        if (curA === curB) {
          Toast.show({
            type: "error",
            text1: "Lỗi",
            text2: "Đang hòa, không thể ghi nhận đúng tỉ số hiện tại.",
          });
      ...
      Toast.show({
        type: "error",
        text1: "Lỗi",
        text2: e?.data?.message || e?.error || "Không thể kết thúc ván sớm",
      });
    }
  };
  ...
  const onAssignedCourt = async ({ courtId, error }) => {
    ...
    Toast.show({
      type: "success",
      text1: courtId ? "Đã gán sân" : "Đã bỏ gán sân",
    });
  };
    ...
    const TeamPanelPortrait = ({ sideKey }) => {
      ...
      return (
        ...
        >
          ...
          {/* Danh sách VĐV + Ô ngay trong thẻ điểm */}
          <View style={{ gap: 4, marginBottom: 6 }}>
            {(isA ? playersA : playersB).map((u, idx) => {
              const uid = userIdOf(u);
              const slotNow = isA ? slotsNowA[uid] : slotsNowB[uid];
              return (
                <PlayerMini
                  key={uid || idx}
                  user={u}
                  slotNow={slotNow}
                  isServer={
                    serverUid && serverUid === uid && servingSide === sideKey
                  }
                  isReceiver={
                    receiverUid &&
                    receiverUid === uid &&
            ...
            <View style={[styles.row, { gap: 6, flexWrap: "wrap" }]}>
              <Ripple
                onPress={() => {
                  ...
                  Toast.show({ type: "success", text1: "Đã đặt đội giao: A" });
                }}
                style={[
                  styles.btnOutline,
                  { paddingVertical: 6 },
              ...
              <Ripple
                onPress={() => {
                  ...
                  Toast.show({ type: "success", text1: "Đã đặt đội giao: B" });
                }}
                style={[
                  styles.btnOutline,
                  { paddingVertical: 6 },
